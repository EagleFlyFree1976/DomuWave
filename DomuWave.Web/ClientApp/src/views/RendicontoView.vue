<template>
  <div>
    ...
    <template v-else>
      <!-- Avviso: nessun consuntivo approvato -->
      <div v-if="!loading && !consuntivo" class="warning-banner" style="margin-bottom:12px">
        ⚠ Nessun budget consuntivo approvato per questo esercizio. Il rendiconto non è disponibile.
      </div>

      <!-- ══ TAB: Conguaglio per unità ══════════════════════════════════════ -->
      <template v-if="activeTab === 'conguaglio'">
...
const activeTab            = ref('conguaglio')
...
watch(selectedFiscalYearId, async () => {
  activeTab.value        = 'conguaglio'
  consuntivoDetail.value = null
  await load()
})

watch(activeTab, async (tab) => {
  if (tab === 'conti' && !consuntivoDetail.value) {
    await loadDetail()
  }
})
...
/* ── Summary card ────────────────────────────────────────────────────────── */
.summary-card { padding: 1rem 1.25rem 1.25rem; margin-bottom: 1rem; }
.summary-meta {
  display: flex;
  gap: 2rem;
  font-size: 0.875rem;
