...
<template>
  ...
  <div v-if="categoryStore.edititem != null">
...
<script setup>
  ...
  const onFormSubmit = async ({ valid }) => {
    ...
    if (valid) {
      ...
      if (categoryStore.createmode) {
        await categoryStore.createEntity(editEntity.value);
      }
      else {
        await categoryStore.updateEntity(editEntity.value);
      }
      saving.value = false;
  ...
  watch(
    () => categoryStore.edititem,
    (newId) => {
      if (newId != null) {

        editEntity.value = { ...categoryStore.edititem };
      }
    },
    { immediate: true }
  )
