...
namespace DomuWave.Services.Command.Category
{
    ...
    // </summary>
    public class CreateImportCommandConsumer : InMemoryConsumerBase<CreateImportCommand, Models.Import.Import>
    {
        private IUserService _userService;
        private IImportService _importService;

        public CreateImportCommandConsumer(ISessionFactoryProvider sessionFactoryProvider, IUserService userService, IImportService importService) : base(sessionFactoryProvider)
        {
        ...
        protected override async Task<Models.Import.Import> Consume(CreateImportCommand evt, IMediationContext mediationContext, CancellationToken cancellationToken)
        {
            var currentUser =
            ...
            var records = await GetRecords(evt.csvStream, cancellationToken).ConfigureAwait(false);


            ImportConfigurationDto configuration = new ImportConfigurationDto();
            configuration.Delimiter = ";";
            configuration.FileName = evt.FileName;
            configuration.HasHeader = evt.HasHeader;
            configuration.Fields = new List<ImportField>();
            ...
            Models.Import.Import import = new Models.Import.Import();
            import.Configuration = configuration.toJSon();
            import.ContentType = "text/json";
