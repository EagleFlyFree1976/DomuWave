...
public class GetWorksByStatusCommandConsumer
    : InMemoryConsumerBase<GetWorksByStatusCommand, IList<ExtraordinaryWorkReadDto>>
{
    private readonly IExtraordinaryWorkService _workService;
    private readonly IUserService              _userService;

    public GetWorksByStatusCommandConsumer(
    ...
    protected override async Task<IList<ExtraordinaryWorkReadDto>> Consume(
        GetWorksByStatusCommand command, IMediationContext mediationContext, CancellationToken cancellationToken)
    {
        var currentUser = await _userService.GetByIdAsync(command.CurrentUserId, cancellationToken).ConfigureAwait(false);
        var list = await _workService.GetByStatusAsync(command.CondominiumId, command.Status, currentUser, cancellationToken).ConfigureAwait(false);
